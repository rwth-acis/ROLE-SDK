...
dojo.addOnLoad(function() {
    ...
    dojo.query(".gadgets-gadget-chrome").forEach(gadgets.my.makeGadgetDnD);
});

gadgets.my = {};
gadgets.my.makeGadgetDnD = function(gadgetNode, belowNode) {
        var m = new dojo.dnd.Moveable(gadgetNode); 
        ...
        dojo.connect(m, "onMoveStart", function(mover){ 
            ...
            dojo.query(".gadgets-gadget-content", belowNode).forEach(function(fc) {
        ...
        dojo.connect(m, "onMoving", function(mover, leftTop) {
            ...
            var targets = dojo.query(".gadgets-gadget-chrome", belowNode)
            .filter (function(x) {
                return x != mover.node;
            })
            .filter(function(x) {
            ...
            if (!targets.length) {
                var col;
				dojo.query(".column", belowNode).forEach(function(column) {
                    var coords = dojo.coords(column);
                    if (coords.l < cx && coords.l + coords.w > cx) col = column;

                    //the column may be empty
                    else if (!dojo.query(".gadgets-gadget-chrome", column).length && cx > coords.l)
                        col = colNum;
                });
                
                if (col && cy > dojo.coords(col).t+dojo.coords(col).h) {
                    dojo.place(placeholder, col, "last");
                } else return;
            } 
        ...
        dojo.connect(m, "onMoveStop", function(mover){ 
            ...
            isMoving = false;
            dojo.query(".dndOverlay", belowNode).orphan();
