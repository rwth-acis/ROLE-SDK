define([ "jquery", "./strophe", "domReady" ], function($, Strophe, domReady) {
...
//                Groupie.connection.disconnect();
                ...
                $("#sideEntry-" + user_jid.split("@")[0].replace('-', '_')).addClass("is-online"); // ROLE-FIX
            	...
            	delete Groupie.participants[nick]; // ROLE-FIX
            	var user_jid = $(presence).find('item').attr('jid'); // ROLE-FIX
            	$("#sideEntry-" + user_jid.split("@")[0].replace('-', '_')).removeClass("is-online"); // ROLE-FIX
...
domReady(function() { //$(document).ready(function () {
	...
	// ROLE-FIX, orig: $('#input').keypress(function (ev) {
    $(document.body).on("keypress", "#input", function (ev) {
        if (ev.which === 13) {
            ev.preventDefault();
	...
	$(document).bind('chat-connected', function () {
	...
	$(document).bind('user_joined', function (ev, nick) {
		// ROLE-FIX (commented the following)
